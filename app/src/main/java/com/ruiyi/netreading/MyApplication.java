...
public class MyApplication extends Application {
    ...
    public void onCreate() {
        ...
        if (!imagecache.exists()) {
            Log.i("MyApplication", "创建图片缓存文件夹");
            imagecache.mkdirs();
        }
        if (!handwriting.exists()) {
            Log.i("MyApplication", "创建笔记缓存文件夹");
            handwriting.mkdirs();
        }
        if (!download.exists()) {
            Log.i("MyApplication", "创建更新包文件夹");
            download.mkdirs();
        }
        //设置默认：自动提交、小数赋分、置顶分数
        ...
        Log.i("MyApplication", "设备型号: " + Tool.getSystemModel());
        Log.i("MyApplication", "设备android版本: " + Tool.getSystemVersion());
